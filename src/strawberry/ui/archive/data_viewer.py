...
class App:
    ...
    def __init__(self):
        ...
        self.trn_store = ParquetStorage(self.config.env.dim_stocks_folder)
    ...
    def render(self):
        ...
        with header_cols[1]:
            self.selected_year_quarter = st.selectbox(
                "Select ticker",
                options=self.tickers,
                index=0,
                key="selected_ticker",
                on_change=lambda: self.ticker_event_handler(
    ...
    def _table_path(self, table_name: str) -> Path:
        return (
            ...
            / self.config.env.acquisition_folder
            / Path(table_name)
        )
    ...
    def startup(self):
        # page configuration
        st.set_page_config(
            page_title="Acquisition Data Viewer",
            layout="wide",
        ...
        for name, r_fn in pages.items():
            ...
            st.sidebar.button(
                label=name, type=btn_type, use_container_width=True, on_click=r_fn
            )
        st.sidebar.button(
            label="validation",
            type="primary",
            use_container_width=True,
            on_click=self.validation_button,
        )
        st.sidebar.button(
            label="dim stocks",
            type="primary",
            use_container_width=True,
            on_click=self.dim_stocks_button,
        )
